...
class Storage<EntityType> {
	...
	constructor(storagePath: string, private pkResolver: PkResolver<EntityType>) {
...
type Packet = { command: 'register', payload: { username: string, secret: string } }
...
const server = net.createServer((socket) => {
	...
	socket.on('data', (data) => {
		...
		if (packet) {
			...
			} else if (connections[socketId].mode === 'in_channel' && connections[socketId].user && connections[socketId].currentChannel) {
				switch (packet.command) {
					case 'send-message':
